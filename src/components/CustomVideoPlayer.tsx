...
export const CustomVideoPlayer = forwardRef<HTMLVideoElement, CustomVideoPlayerProps>(
  ({ src, poster, aspectRatio = 'horizontal', showInitialPlayIcon = false, startMuted = false }, ref) => {
    ...
    // Controles visíveis enquanto o usuário interage (essencial em telas de
    // toque, onde não existe hover) e alguns segundos após cada interação.
      ...
      if (container) {
        if (!document.fullscreenElement) {
          container.requestFullscreen().catch(err => {
            // Feedback inline em vez de alert() bloqueante.
            setFullscreenError(`Erro ao entrar em tela cheia: ${err.message} (${err.name})`);
          });
        } else {
    ...
    useEffect(() => {
      ...
      const handleVideoEnd = () => {
        setIsPlaying(false);
        // Ao terminar, deixa os controles visíveis para o usuário reassumir.
        setControlsVisible(true);
      };
        ...
        aspectRatioClass
          ...
          <div className={cn(
            ...
            isPlaying
              ...
              {/* BARRA DE CONTROLES — visível por 3s após cada interação (desktop
                  também revela no hover do grupo); clique na barra ajusta o tempo. */}
              <div
                className={cn(
                  "w-full p-2 sm:p-4 pointer-events-auto",
                  !isPlaying && controlsVisible && "sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300"
                )}
              >
          ...
          {fullscreenError && (
            <div
              role="alert"
              className="absolute top-3 left-1/2 -translate-x-1/2 bg-black/80 text-white text-xs sm:text-sm px-4 py-2 rounded-lg border border-red-500/40 max-w-[90%] text-center"
            >
              {fullscreenError}
              <button
