...
describe('CustomVideoPlayer', () => {
  ...
  it('should handle fullscreen errors gracefully', async () => {
    const user = userEvent.setup()
    // Mock do window.alert: o player não deve usar alert() bloqueante.
    ...
    // Não deve quebrar mesmo com erro, e o erro deve aparecer inline (role=alert)
    expect(fullscreenButton).toBeInTheDocument()
    expect(alertSpy).not.toHaveBeenCalled()

    const inlineAlert = await screen.findByRole('alert')
    expect(inlineAlert).toHaveTextContent(/erro ao entrar em tela cheia/i)
    expect(inlineAlert).toHaveTextContent(/fullscreen error/i)
